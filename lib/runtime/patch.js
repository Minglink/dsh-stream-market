 ...
 */
export function readPatchFileState(profileDir) {
 ...
 */
export function setPluginPatchState(profileDir, pluginId, enabled) {
    const patchPath = join(profileDir, 'cordis.patch.yml');
    let patchList = [];
    if (existsSync(patchPath)) {
        try {
            ...
            const parsed = yaml.load(raw);
            if (Array.isArray(parsed)) {
                patchList = parsed.filter(i => i && typeof i === 'object');
            }
        }
        catch {
            patchList = [];
 ...
 */
export function removePluginPatchEntry(profileDir, pluginId) {
    const patchPath = join(profileDir, 'cordis.patch.yml');
    if (!existsSync(patchPath))
        return true;
    try {
