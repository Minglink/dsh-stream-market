...
import {
  parseDshProtocolUrl,
  buildDshProtocolUrl,
  executeProtocolInstall,
  generateHtmlBadge,
  generateMarkdownBadge,
} from '../runtime/protocol.js'
import type { MarketConfig, DshProtocolInstallParams } from '../types.js'
...
export class MarketBridgeServer {
  ...
  public async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (pathname === '/api/market/clean-uninstall' && (req.method === 'POST' || req.method === 'GET')) {
      ...
      const executeClean = (targetIds: string[], opts: any) => {
        if (!targetIds.length) {
          res.writeHead(400, { 'Content-Type': 'application/json' })
        ...
        if (isStream) {
          ...
          const onData = (evt: any) => res.write(`data: ${JSON.stringify(evt)}\n\n`)
    ...
    if (pathname === '/api/market/protocol/build' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          const params = JSON.parse(body || '{}') as DshProtocolInstallParams
