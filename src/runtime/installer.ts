...
import type { InstallProgressEvent, CleanUninstallOptions } from '../types.js'
...
export class PluginInstaller {
  private activeTasks: Map<string, InstallTask> = new Map()

  /**
   * 安装插件并返回一个事件流
   */
  public install(pluginId: string, source: string, profile: string = 'web'): EventEmitter {
   ...
   */
  public uninstall(pluginId: string, profile: string = 'web'): EventEmitter {
   ...
   */
  public cleanUninstall(
    pluginIds: string[],
    profile: string = 'web',
    options: Partial<CleanUninstallOptions> = {}
  ): EventEmitter {
    const emitter = new EventEmitter()
    ...
    setImmediate(async () => {
      const emitLog = (msg: string, type: 'log' | 'success' | 'error' = 'log') => {
        emitter.emit('data', { type, message: msg } as InstallProgressEvent)
      }
  ...
  private runInstallProcess(task: InstallTask) {
    const { id, source, profile, eventEmitter } = task
    const profileDir = resolveProfileDir(profile)
    const taskKey = `${profile}:${id}`

    const emitLog = (msg: string, stage: InstallProgressEvent['stage'] = 'installing') => {
    ...
    try {
      ...
      proc.on('close', (code) => {
        ...
        try { rescueDshBundles(profile); } catch {}
          eventEmitter.emit('data', {
            type: 'success',
  ...
  private runFallbackPackageManager(task: InstallTask, profileDir: string) {
    const { id, source, profile, eventEmitter } = task
    const taskKey = `${profile}:${id}`

    const emitLog = (msg: string) => {
      eventEmitter.emit('data', { type: 'log', message: msg } as InstallProgressEvent)
    }
  ...
  private runNpmUninstall(pluginId: string, profileDir: string, emitter: EventEmitter) {
    const isWindows = process.platform === 'win32'
    const npmCmd = isWindows ? 'npm.cmd' : 'npm'
