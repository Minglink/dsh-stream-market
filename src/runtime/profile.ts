 ...
 */
export function resolveProfileDir(profileName: string = 'web'): string {
  // 1. 优先环境变量
 ...
 * 获取 DSH 宿主环境运行状态
 */
export function getHostEnvironment(profileName: string = 'web'): HostEnvironment {
 ...
 */
export function scanInstalledPlugins(profileName: string = 'web'): Record<string, InstalledPluginInfo> {
  const profileDir = resolveProfileDir(profileName)
  const results: Record<string, InstalledPluginInfo> = {}
 ...
 */
export function isCoreSystemPackage(name: string): boolean {
 ...
 * 判定包名是否为 DSH 相关插件
 */
export function isDshPluginName(name: string): boolean {
 ...
 */
export function rescueDshBundles(profileName: string = 'web'): number {
  const profileDir = resolveProfileDir(profileName)
  const pkgJsonPath = join(profileDir, 'package.json')
